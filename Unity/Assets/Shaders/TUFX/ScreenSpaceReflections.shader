Shader "Hidden/TUFX/ScreenSpaceReflections"
{
    CGINCLUDE
        ...
        float3 GetViewSpacePosition(float2 uv)
        {
            ...
            return result.xyz / max(0.00001, result.w);
        }
        ...
        float3 ReconstructForwardNormal(float2 uv)
        {
            float2 texel = _MainTex_TexelSize.xy;
            float3 cP = GetViewSpacePosition(uv);
            float3 lP = GetViewSpacePosition(uv + float2(-texel.x, 0));
            float3 rP = GetViewSpacePosition(uv + float2( texel.x, 0));
            float3 tP = GetViewSpacePosition(uv + float2(0,  texel.y));
            float3 bP = GetViewSpacePosition(uv + float2(0, -texel.y));
            ...
            float lenSq = dot(n, n);
            if (lenSq < 0.00001) return float3(0, 0, -1);
            return normalize(n);
        }
        ...
        Result March(Ray ray, VaryingsDefault input)
        {
            ...
            const float2 homogenizers = rcp(float2(max(0.00001, r.w), max(0.00001, q.w)));
            ...
            for (int i = 0; i < _MaximumIterationCount; ++i)
            {
                ...
                float d = _CameraDepthTexture.SampleLevel(sampler_CameraDepthTexture, UnityStereoTransformScreenSpaceTex(uv), 0).r;
        ...
        float4 FragTest(VaryingsDefault i) : SV_Target
        {
            ...
            if (_IsDeferred > 0.5)
            {
                ...
                if (dot(gbuffer2.rgb, 1.0) == 0.0)
                    return 0.0;

                normal = 2.0 * gbuffer2.rgb - 1.0;
                normal = mul((float3x3)_ViewMatrix, normal);
            }
            else
            {
                float depth = _CameraDepthTexture.SampleLevel(sampler_CameraDepthTexture, i.texcoordStereo, 0).r;
                #if UNITY_REVERSED_Z
                ...
                normal = ReconstructForwardNormal(i.texcoord);
            }
        ...
        // Pass 3: Composite SSR onto Scene
        float4 FragComposite(VaryingsDefault i) : SV_Target
        {
            ...
            if (_IsDeferred > 0.5)
            {
                ...
                if (dot(gbuffer2.rgb, 1.0) > 0.0)
                {
                    ...
                    resolve.a *= 2.0 * resolve.a;
                    float fade = 1.0 - saturate(1.5 * resolve.a * smoothstep(0.5, 1.0, 1.5 * resolve.a) * _DistanceFade);

                    resolve.rgb = lerp(reflectionProbes.rgb, resolve.rgb, confidence * fade * _ReflectionIntensity);
            ...
            // Forward SSSR Composite Mode (depth normal reconstruction + Fresnel)
            float3 fwdPos = GetViewSpacePosition(i.texcoord);
            float3 fwdV = -normalize(fwdPos);
            float3 fwdN = ReconstructForwardNormal(i.texcoord);

            float4 resolveFwd = _Resolve.SampleLevel(sampler_Resolve, i.texcoordStereo, 0);

            float NdotV = saturate(dot(fwdN, fwdV));
            float bias = saturate(_ForwardPbrBias);
            float fresnel = bias + (1.0 - bias) * pow(1.0 - NdotV, 5.0);

            float conf = resolveFwd.a;
            conf *= 2.0 * conf;
            float fwdFade = 1.0 - saturate(1.5 * conf * smoothstep(0.5, 1.0, 1.5 * conf) * _DistanceFade);

            color.rgb += resolveFwd.rgb * (fresnel * fwdFade * _ReflectionIntensity);
            return color;
        }
